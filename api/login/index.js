...
const { safeEqual, mintJWT, mintPendingJWT } = require('./core');
...
module.exports = async function login(context, req) {
  ...
  try {
    ...
    if (totpSecret) {
      const mfaToken = mintPendingJWT(jwtSecret, MFA_PENDING_TTL);
      context.res = { status: 200, headers: CORS, body: { mfaRequired: true, mfaToken } };
      return;
    }
